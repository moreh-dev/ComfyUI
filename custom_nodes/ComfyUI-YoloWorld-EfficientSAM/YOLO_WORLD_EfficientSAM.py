...
folder_paths.folder_names_and_paths["yolo_world"] = (
    [os.path.join(folder_paths.models_dir, "yolo_world")],
...
class ESAM_ModelLoader_Zho:
    ...
    def load_esam_model(self, device):
        ...
        if device == "CUDA":
            model_path = os.path.join(current_directory, "efficient_sam_vits.pt")
        else:
            model_path = os.path.join(current_directory, "efficient_sam_vits.pt")

        # EFFICIENT_SAM_MODEL = torch.jit.load(model_path)
        EFFICIENT_SAM_MODEL = build_efficient_sam_vits().cuda()
        EFFICIENT_SAM_MODEL.load_state_dict(torch.load(model_path)["model"])
        ...
        for img in image:
            ...
            if with_segmentation:
                detections.mask = inference_with_boxes(
                    image=img, xyxy=detections.xyxy, model=esam_model, device=DEVICE
                )
                if mask_combined:
