...
GPU_EFFICIENT_SAM_CHECKPOINT = "efficient_sam_vits.pt"
CPU_EFFICIENT_SAM_CHECKPOINT = "efficient_sam_vits.pt"


def load(device: torch.device) -> torch.jit.ScriptModule:
    if device.type == "cuda":
        model = build_efficient_sam_vits()
        model.load_state_dict(torch.load(GPU_EFFICIENT_SAM_CHECKPOINT)["model"])
        model.cuda()
    else:
        model = build_efficient_sam_vits()
        model.load_state_dict(torch.load(GPU_EFFICIENT_SAM_CHECKPOINT)["model"])
