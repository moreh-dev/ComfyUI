    ...
    def __call__(self, denoiser, x, cond, uc=None, num_steps=None, control_scale=1.0, **kwargs):
        ...
        for _idx, i in enumerate(self.get_sigma_gen(num_sigmas)):
            ...
            for j, (hi, hi_end, wi, wi_end) in enumerate(latent_tiles_iterator):
                ...
                x_next[:, :, hi:hi_end, wi:wi_end] = x_next[:, :, hi:hi_end, wi:wi_end] + _x * tile_weights
                old_denoised_next[:, :, hi:hi_end, wi:wi_end] = old_denoised_next[:, :, hi:hi_end, wi:wi_end] + _old_denoised * tile_weights
                count[:, :, hi:hi_end, wi:wi_end] =  count[:, :, hi:hi_end, wi:wi_end] + tile_weights
